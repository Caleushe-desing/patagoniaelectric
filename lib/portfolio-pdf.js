...
function buildPortfolioPdf(portafolio, site) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // ── Projects (misma lista que la web: proyectos.items) ──
    const projectList = Array.isArray(portafolio._sharedProjects)
      ? portafolio._sharedProjects
      : (portafolio.projects || []);
    ...
    // ── Clients (misma lista que la web: clientes.items) ──
    const clientLogos = Array.isArray(portafolio._sharedClients)
      ? portafolio._sharedClients
      : (portafolio.clients || []);
    if (clientLogos.length) {
      ...
      const names = clientLogos.map((c) => c.name).filter(Boolean);
